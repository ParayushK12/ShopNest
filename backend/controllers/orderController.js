...
export const createOrder = async (req, res) => {
  ...
  try {
    ...
    // 1. Safe Stock Enforcement: Validate availability and decrement inventory atomically
    for (const item of products) {
      ...
      if (!updatedProduct) {
        throw new Error(`Product with ID ${item.product} is out of stock or does not exist`);
      }
    ...
    try {
      ...
      // Rollback stock decrement if order creation fails
    ...
    // 3. Populate order details for email
    const populatedOrder = await Order.findById(newOrder._id).populate("products.product");

    // 4. Send Confirmation Email (wrapped to prevent mail failure from failing the order)
    try {
